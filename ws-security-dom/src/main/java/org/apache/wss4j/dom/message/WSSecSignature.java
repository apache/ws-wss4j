...
public class WSSecSignature extends WSSecSignatureBase {
    ...
    private X509Certificate[] getSigningCerts() throws WSSecurityException {
        ...
        if (!(keyIdentifierType == WSConstants.CUSTOM_SYMM_SIGNING
            ...
            if (sigAlgo == null) {
                ...
                } else if (pubKeyAlgo.equalsIgnoreCase("EdDSA")) { // JRE16+
    ...
    /**
     * The method returns a public key algorithm for an EdDSA key type (Ed25519 or Ed448). The method does not rely on
     * a public key interface class to support older JRE versions than JRE16 where EdCA keys were not yet introduced.
     * Instead, it parses the DER-encoded public key to determine the algorithm.
     *
     * @param publicKey the public key to get the algorithm from
     * @return the algorithm (ED25519 or ED448) for the public key
