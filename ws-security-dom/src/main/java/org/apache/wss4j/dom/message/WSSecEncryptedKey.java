...
public class WSSecEncryptedKey extends WSSecBase {
     ...
     * Example for ECDH-ES: Chttp://www.w3.org/2009/xmlenc11#ECDH-ES
     *
     */
    ...
    public void prepare(Crypto crypto, SecretKey symmetricKey) throws WSSecurityException {
        ...
        if (useThisPublicKey != null) {
            ...
            Key kek;
            KeyAgreementParameterSpec dhSpec = null;
     ...
     */
    protected void createEncryptedKeyElement(X509Certificate remoteCert, Crypto crypto, KeyAgreementParameterSpec dhSpec)
            throws WSSecurityException {
     ...
     */
    public KeyAgreementParameterSpec buildKeyAgreementParameter(PublicKey recipientPublicKey)
            throws  WSSecurityException {
        KeyAgreementParameterSpec dhSpec;
        try {

            int keyBitLength  = org.apache.xml.security.utils.KeyUtils.getAESKeyBitSizeForWrapAlgorithm(keyEncAlgo);
            KeyDerivationParameter kdf = AlgorithmParameterUtils.buildConcatKeyDerivationParameter(keyBitLength,  digestAlgo);
            KeyPair dhKeyPair = org.apache.xml.security.utils.KeyUtils.generateEphemeralDHKeyPair(recipientPublicKey, null);
            dhSpec = AlgorithmParameterUtils.buildAgreementParameters(keyAgreementMethod,
                    KeyAgreementParameterSpec.ActorType.ORIGINATOR, kdf, null, recipientPublicKey);
     ...
     *
     * @param keyAgreementParameter the {@link KeyAgreementParameterSpec} for generating the secret key
     ...
     */
    public SecretKey generateEncryptionKey(KeyAgreementParameterSpec keyAgreementParameter) throws WSSecurityException {
        try {
    ...
    protected byte[] encryptSymmetricKey(Key encryptingKey, SecretKey keyToBeEncrypted)
        ...
        try {
            ...
            if (WSConstants.KEYTRANSPORT_RSAOAEP.equals(keyEncAlgo)
                ...
                oaepParameterSpec = AlgorithmParameterUtils.buildOAEPParameters(keyEncAlgo, digestAlgo, mgfAlgo, null);
            }
