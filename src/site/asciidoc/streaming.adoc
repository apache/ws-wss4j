...
password-less UsernameToken for an sp:NoPassword policy, or rsa-1_5 key transport
 ...
 * The REQUIRE_SIGNED_ENCRYPTED_DATA_ELEMENTS ("requireSignedEncryptedDataElements")
...
effect and reports nothing. A deployment that switches from the DOM interceptors
