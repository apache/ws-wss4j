...
public class WSSecEncrypt extends WSSecBase {
    ...
    public void setupToken(Document doc, Crypto crypto, Element securityHeader)
        ...
        /*
         * Get the certificate that contains the public key for the
         ...
         * 3) Create and set up the SecurityTokenReference according to the keyIdentifer
         *    parameter
         * 4) Create the CipherValue element structure and insert the encrypted
         *    session key
         * 5) The last step sets up the reference list that pints to the encrypted
        ...
        String prefix = WSSecurityUtil.setNamespace(xencEncryptedKey, WSConstants.WSU_NS, WSConstants.WSU_PREFIX);
        xencEncryptedKey.setAttributeNS(WSConstants.WSU_NS, prefix + ":Id", encKeyId);
    ...
    public void prependEncryptElementToHeader(Element securityHeader) {
        WSSecurityUtil.prependChildElement(document, securityHeader, xencEncryptedKey, false);
    }
    ...
    public Element encryptForExternalRef(Element dataRef, Vector references) throws WSSecurityException {
        ...
        Vector encDataRefs = doEncryption(document, this.encryptionKey, keyInfo, parts);
        Element referenceList = dataRef;
		if (referenceList == null) {
			referenceList = document.createElementNS(WSConstants.ENC_NS,
					WSConstants.ENC_PREFIX + ":ReferenceList");
    ...
    public void addRefElement(Element referenceList) {
        WSSecurityUtil.appendChildElement(document, xencEncryptedKey, referenceList);    	
    }
    ...
    private Vector doEncryption(Document doc,
                                SecretKey secretKey,
                                KeyInfo keyInfo, Vector references)
            throws WSSecurityException {
    ...
    private Document buildEmbedded(Document doc, Crypto crypto, WSSecHeader secHeader)
            throws WSSecurityException {
        doDebug = log.isDebugEnabled();
        ...
        Vector encDataRefs = doEncryption(doc, this.encryptionKey, keyInfo, parts);
    ...
    public static Element createDataRefList(Document doc, Element referenceList,
                                            Vector encDataRefs) {
